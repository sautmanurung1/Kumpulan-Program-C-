...
namespace sorting
{
    void bubbleSort(std::vector<int> &numbers)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                // jika current number yang direprsentasikan oleh numbers[x] lebih besar dari
                // nextNumber numbers[x+1] maka tukarkan posisi 2 angka tersebut
                if (numbers[x] > numbers[x + 1])
                {
                    // std::cout << "Menukar: " << numbers[x] << " Dengan " << numbers[x+1] << std::endl;
                    auto temp = numbers[x];
